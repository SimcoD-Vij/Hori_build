...
# Install torch CPU and all deps in ONE RUN to prevent BuildKit layer cache
# from reusing a GPU torch layer and silently pulling CUDA (saves ~2 GB).
# pocket-tts pinned to tested version — bump intentionally after validation.
