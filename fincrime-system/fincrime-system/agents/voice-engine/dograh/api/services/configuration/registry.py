...
class BaseServiceConfiguration(BaseModel):
    provider: Literal[
        ...
        ServiceProviders.QWENTTS,
        ServiceProviders.OLLAMA,
        ServiceProviders.POCKETTTS,
...
SPEACHES_LLM_MODELS = ["llama3.2:1b", "llama3.2:3b", "llama3.2", "llama3", "mistral", "phi3", "qwen2", "gemma2", "deepseek-r1"]
...
OLLAMA_LLM_MODELS = [
    "llama3.2:3b-instruct-q4_K_M",  # 🏆 Recommended — Fits in RAM & follows instructions
    "llama3.2:1b-instruct-q4_K_M",  # ⚡ Ultra-fast — but may fail instructions
    "llama3.1:8b",   # High quality (requires ~5 GB RAM)
    "qwen2.5:7b",    # Good alternative with tool support
    "llama3.2:3b",   # Lightweight but limited tool calling
    "mistral:7b",
    "phi3:medium",
]

@register_llm
class OllamaLLMConfiguration(BaseLLMConfiguration):
    provider: Literal[ServiceProviders.OLLAMA] = ServiceProviders.OLLAMA
    model: str = Field(
        default="llama3.2:3b-instruct-q4_K_M",
        json_schema_extra={
            "examples": OLLAMA_LLM_MODELS,
            "allow_custom_input": True,
        },
    )
    base_url: str = Field(
        default="http://ollama:11434/v1",
        description="Ollama OpenAI-compat endpoint (must end in /v1)",
    )
    api_key: str | list[str] | None = Field(
        default="ollama",
        description="Any string; Ollama ignores API keys"
    )
...
@register_tts
class QwenTTSConfiguration(BaseTTSConfiguration):
    provider: Literal[ServiceProviders.QWENTTS] = ServiceProviders.QWENTTS
    api_url: str = Field(description="Custom Qwen3-TTS endpoint URL")
    voice: str = Field(default="", description="Voice ID (Cloned)")
    model: str = Field(default="qwen-tts")
    api_key: str | list[str] | None = Field(default=None)


@register_tts
class PocketTTSConfiguration(BaseTTSConfiguration):
    provider: Literal[ServiceProviders.POCKETTTS] = ServiceProviders.POCKETTTS
    api_url: str = Field(default="http://pocket-tts:8000", description="URL of the pocket-tts server")
    voice: str = Field(default="alba", description="Voice Selection (Cloned path or Default name)")
    model: str = Field(default="pocket-tts-v1", enum=["pocket-tts-v1"], description="Pocket TTS Model version")
    use_enhanced_pipeline: bool = Field(default=True, description="Enable Layer 1-3 prosody and quality enhancements")
    api_key: str | list[str] | None = Field(default=None, description="Optional API key for connectivity verification")
