...
from api.constants import MPS_API_URL, OLLAMA_BASE_URL
...
from pipecat.services.qwen_tts import QwenTTSService
...
def create_tts_service(user_config, audio_config: "AudioConfig"):
    ...
    elif user_config.tts.provider == ServiceProviders.QWENTTS.value:
        return QwenTTSService(
            api_url=getattr(user_config.tts, "api_url", None),
            voice_id=getattr(user_config.tts, "voice", None),
            api_key=getattr(user_config.tts, "api_key", None),
            text_filters=[xml_function_tag_filter],
            skip_aggregator_types=["recording_router", "recording"],
        )
    elif user_config.tts.provider == ServiceProviders.POCKETTTS.value:
        return PocketTTSService(
            api_url=getattr(user_config.tts, "api_url", "http://pocket-tts:8000"),
            voice_id=getattr(user_config.tts, "voice", None),
            use_enhanced_pipeline=getattr(user_config.tts, "use_enhanced_pipeline", True),
            text_filters=[xml_function_tag_filter],
            skip_aggregator_types=["recording_router", "recording"],
        )
    else:
...
def create_llm_service_from_provider(
    ...
    *,
    ...
    elif provider == ServiceProviders.OLLAMA.value:
        _ollama_model = model or "llama3.2:1b-instruct-q4_K_M"
        _ollama_base = (base_url or f"{OLLAMA_BASE_URL}/v1").rstrip("/")
        return OLLamaLLMService(
            base_url=_ollama_base,
            settings=OllamaLLMSettings(
                model=_ollama_model,
                temperature=0.7,   # Higher temp = more natural, fluid speech
                max_tokens=150,
            ),
        )
    else:
