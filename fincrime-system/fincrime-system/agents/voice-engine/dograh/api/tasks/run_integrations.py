...
async def run_integrations_post_workflow_run(_ctx, workflow_run_id: int):
    ...
    try:
        ...
        if workflow_run:
            ...
            logger.info(f"Workflow Run Context Type: {type(workflow_run.initial_context)}")
            logger.info(f"Workflow Run Context: {workflow_run.initial_context}")
            logger.info(f"Workflow Object: {workflow_run.workflow}")
            if workflow_run.workflow:
                logger.info(f"Workflow Def Type: {type(workflow_run.definition.workflow_json)}")
        else:
            logger.warning(f"Workflow Run {workflow_run_id} NOT FOUND")

        if not workflow_run or not workflow_run.workflow:
            logger.warning(f"Workflow run or workflow not found for run {workflow_run_id}")
            return

        if not organization_id:
            logger.warning(f"No organization found for run {workflow_run_id}, skipping integrations")
            return
        ...
        dynamic_webhook_url = workflow_run.initial_context.get("webhook_url") if workflow_run.initial_context else None
        
        logger.info(f"Integration nodes: {len(webhook_nodes)} webhooks, {len(qa_nodes)} QA nodes. Has campaign: {has_campaign}. Dynamic webhook: {dynamic_webhook_url}")
        
        if not webhook_nodes and not qa_nodes and not has_campaign and not dynamic_webhook_url:
            logger.info("No integration nodes, no campaign, and no dynamic webhook, skipping")
            return

        public_token = None
        if webhook_nodes or has_campaign or dynamic_webhook_url:
        ...
        logger.info(f"Initial context for run {workflow_run_id}: {workflow_run.initial_context}")
        dynamic_webhook_url = workflow_run.initial_context.get("webhook_url") if workflow_run.initial_context else None
        if not webhook_nodes and not dynamic_webhook_url:
            logger.debug("No webhook nodes and no dynamic webhook, skipping")
            return
        ...
        # Step 9: Execute dynamic webhook if present in initial_context
        if dynamic_webhook_url:
            logger.info(f"### DYNAMIC WEBHOOK START ### URL: {dynamic_webhook_url}")
            if 'render_context' not in locals():
                render_context = _build_render_context(workflow_run, public_token)
            
            # Re-fetch gathered_context to ensure we have latest (like call_disposition)
            dynamic_webhook_data = {
                "name": "Dynamic Completion Webhook",
                "endpoint_url": dynamic_webhook_url,
                "enabled": True,
                "http_method": "POST",
                "payload_template": {
                    "workflow_run_id": "{{workflow_run_id}}",
                    "workflow_run_name": "{{workflow_run_name}}",
                    "duration_seconds": "{{cost_info.call_duration_seconds | fallback:0}}",
                    "recording_url": "{{recording_url}}",
                    "transcript_url": "{{transcript_url}}",
                    "transcript": "{{transcript}}",
                    "disposition": "{{gathered_context.call_disposition | fallback:COMPLETED}}",
            ...
            try:
                await _execute_webhook_node(
                    webhook_data=dynamic_webhook_data,
                    render_context=render_context,
                    organization_id=organization_id,
                )
            except Exception as e:
                logger.warning(f"Failed to execute dynamic webhook: {e}")
...
def _build_render_context(
    ...
    # Format transcript as a list of turns for the CRM
    events = (workflow_run.logs or {}).get("realtime_feedback_events", [])
    transcript_turns = []
    for event in events:
        ...
        if (
            ...
            transcript_turns.append({"role": "user", "content": payload.get("text", "")})
        elif event_type == RealtimeFeedbackType.BOT_TEXT.value:
            transcript_turns.append({"role": "assistant", "content": payload.get("text", "")})
